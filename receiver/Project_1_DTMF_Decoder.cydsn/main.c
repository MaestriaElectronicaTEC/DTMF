...
{   
    ...
    if(SendSingleByte || ContinuouslySendData)
    {
        ...
        goertzel_input[sampler_counter] = output;
        sprintf(TransmitBuffer, "Goertzel:%u, sampler_counter: %ui\r\n", goertzel_input[sampler_counter], sampler_counter);
...
double goertzel_filter(double in_signal[], double frecuency_detect, uint16 samples)
{
    ...
    cos_coeff = 2*cos(2*3.14*frecuency_detect/SAMPLE_FRECUENCY); 
